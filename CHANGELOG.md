...
### Added
- **An `other` lane for downloads that are neither TV nor a movie.** Prowlarr's search page had a download button that did nothing — Prowlarr had no download clients of its own, so a grab produced no event, no error and no History entry; only Sonarr and Radarr could ever send anything anywhere. `configure-apps.sh` now gives Prowlarr qBittorrent and SABnzbd as its own clients (both `localhost` — it shares Gluetun's namespace) with category `other`, creates that category in both clients (`/data/torrents/other`, `/data/usenet/complete/other`), and the e2e suite asserts Prowlarr's clients pass their own connection test and the categories exist. Nothing imports from `other`; it is a landing spot, and it inherits the executable exclusions and the scan.
- **`--only <section>`** runs one section of `configure-apps.sh` — `bazarr`, `qbittorrent`, … — and touches nothing else. `BAZARR_CONTAINER=<throwaway> --only bazarr` is now a genuinely isolated test; before, the documented "test against a throwaway" workflow still reconfigured the live qBittorrent, Sonarr, Radarr, Prowlarr and Pi-hole, and setting the container without `BAZARR_PORT` read the throwaway's API key while writing to the live port. The port is now read from `docker port`.
- **Tests for the HTTP layer and the language plan**: `tests/configure-helpers.bats` stubs curl to prove a refused connection is not a ✓, a timeout returns 2 with the diagnosis, a dry run sends nothing and a bad timeout value is refused; `tests/bazarr-language-plan.bats` drives the plan through every profile state above; the e2e suite asserts the live Bazarr has an English profile that both defaults point at.

### Fixed
- **`configure-apps.sh` wrote `localhost` for every cross-namespace connection, which reaches nothing on a fresh install.** Sonarr and Radarr live on the bridge since the VPN migration, so their download clients must be `gluetun:8085`/`gluetun:8080`, and Prowlarr — inside Gluetun's namespace, where DNS is Pi-hole — must reach them by static IP (`172.20.0.10`/`.11`) with `http://gluetun:9696` as its callback URL. `docs/MIGRATION-arr-off-vpn.md` had listed exactly those values; the script never got them, and passed on this stack only because it skips what already exists. Verified against the live configs, which were corrected by hand at migration time. The REFERENCE connection table said `sonarr:8989` for the same hop; corrected to the IP.
- **Bazarr's subtitle profile is managed by name or contents, and both defaults point at its real id.** `configure-apps.sh` manages the profile named `English` — adopting an existing profile whose languages match whatever it's called, or creating one with the next free id — makes its languages exactly `SUBTITLE_LANGUAGES` (default `en`; add or remove a language and the script follows), and never touches any other profile. It runs *before* the Sonarr/Radarr connections, because Bazarr gives a series or movie its profile when it first inserts the row and connecting starts that sync at once. Before: a profile was created only when the list was empty, the defaults were hard-coded to id 1 (which reported ✓ against a profile that didn't exist), every profile was pruned to English, and the profile came after the connections — so a fresh install's whole library ended up with no profile. The decision is `scripts/lib/bazarr-language-plan.py`, unit-tested against each of those states.
- **A failed write is reported as a failure.** `_api_request` returned the HTTP status as its exit code, and bash reads curl's `000` — its write-out on a failed connection — as 0, so every `if api_post …; then ok` was green on connection-refused. Both wrappers now share one bounded curl capture (`--connect-timeout 5`, `--max-time` from `API_TIMEOUT`, default 60 s; `BAZARR_SCAN_TIMEOUT`, default 600 s, for the one Bazarr write that rescans the library) and return 1 on any failure; a timeout prints what may have happened and which knob raises the bound. Knob values are validated — `90s` used to make curl print a usage error, `0` silently removed the bound. The qBittorrent and SABnzbd sections' own curls are bounded the same way.
- **No `docker restart bazarr` after the Bazarr section, and no hang.** Bazarr applies settings inside the POST and never restarts its process for them (v1.6.0 source: the only restarts on that path are its SignalR clients). The bounce cost the ~40 s healthcheck window on every fresh run and, once writes were bounded, could fire into a request Bazarr was still executing. `docs/APP-CONFIG-ADVANCED.md`'s contract, which said the opposite, is corrected.
- **`configure-apps.sh` exits non-zero when any step failed.** It printed "Some steps failed" and then exited 0, so a cron line or CI step reading its status saw success.
- **`--help` printed a truncated header.** It used fixed `head`/`tail` offsets, so the "What stays manual after this script" section fell off the end the moment lines were added above it. It now prints the whole comment block.
...
### Added
- **`check-vpn.sh` runs every 5 minutes from cron into an Uptime Kuma push monitor**, which alerts when the pings *stop* — so it also catches the check not running at all. `detect-vpn-zombies.sh` would have caught this release's namespace breakage; nothing invoked it.
- **A DNS probe in `check-vpn.sh`**, covering what Gluetun's health check deliberately stopped watching. It runs before the egress checks, which all resolve hostnames and would otherwise blame egress for a DNS fault.
- **Tests harvested from `feat/harvest-e2e-coverage`** — unit tests for `detect-vpn-zombies.sh`, a live namespace check in the e2e suite, *arr reachability assertions, and shellcheck over the repo's own scripts.

### Fixed
- **Gluetun's `cap_drop: ALL` was removing CHOWN and DAC_OVERRIDE, which it needs.** `VPN_TYPE=openvpn` could never start (`chown /etc/openvpn/target.ovpn: operation not permitted`, retrying forever), and on *every* VPN type Gluetun couldn't write `/tmp/gluetun/ip` — 347 permission-denied errors in the running container, and `/v1/publicip/ip` returning an empty string throughout. Adding only `NET_ADMIN` is what every guide says, and it's correct, but only if you kept Docker's default capability set.
- **A Pi-hole wobble could tear down the VPN and never recover.** The health check resolved `github.com`/`cloudflare.com` by name, through Pi-hole, so a stall was indistinguishable from a dead tunnel — and Gluetun answers a failed health check by rebuilding the tunnel, which cannot fix DNS. On 5 August: 323 VPN rebuilds and 10 container restarts in under two hours, ending only when the container was restarted by hand. `HEALTH_TARGET_ADDRESSES` now pins the check to bare IPs.
- **`check-vpn.sh` exited 0 when it couldn't measure a service at all.** An empty egress result printed `WARN` and was skipped without touching the exit code, so the run in which all four tunneled services had lost their network still ended on "OK: every tunneled service egresses through Gluetun". The script advertises cron use, where a 0 is silence.
- **Seerr's server ids are probed rather than hardcoded to `/0`.**

### Changed
- **Image bump** — `sabnzbd` 5.1.1 → 5.1.2.
- **Never point a container at a `.lan` name** (`docs/REFERENCE.md`). They all resolve to Traefik's macvlan address, which is unreachable from the container bridge by design. Found because Sonarr's Home Assistant webhook was set to one and had been failing silently for days; Radarr already had it right.
- **The UGOS crontab trap** (`docs/UTILITIES.md`). Writing `/var/spool/cron/crontabs/<user>` directly looks like it works — the write succeeds and `crontab -l` lists the entry — but cron never reloads it and the job silently never runs. Needs `sudo crontab -u <user> <file>`.
